...
class winCCData():
    ...
    def extractData(self):
        ...
        self.timeHours = np.divide(self.timeSeconds,60)
    
    def unfoldPulseCount(self):
        for i in range(0,len(self.Npulses)-1):
            if self.Npulses[i] < self.Npulses[i+1]:
                self.Npulses[i+1:] = self.Npulses[i+1:] + self.Npulses[i] - self.Npulses[i+1] + 1
...
def main():  
    ...
    winccPath = r"G:\Departments\TE\Groups\ABT\Sections\PPE\Users\objorkqv\winCC export\20Feb2020_23Feb2020.csv"
