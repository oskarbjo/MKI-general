...
plt.figure()
pickupSignal = signal_toolbox.Signal(data1[:,0],data1[:,1],1)
plt.plot(np.divide(pickupSignal.freq,1e6),20*np.log10(np.abs(pickupSignal.sig_fft)/np.max(np.abs(pickupSignal.sig_fft))))
plt.ylabel('Normalized magnitude')
plt.xlabel('Frequency [MHz]')
plt.grid()

plt.figure()
plt.plot(np.multiply(pickupSignal.t,1e6),pickupSignal.signal)
plt.xlim([0,15])
plt.xlabel('Time [us]')
